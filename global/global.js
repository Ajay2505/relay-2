...
const calc = () => {
    ...
    function platforms() {
        ...
        element.style.background = `linear-gradient(to right, #5ab9ea 0%, #5ab9ea ${per}%, #84ceeb ${per}%, #84ceeb 100%)`;
        valuesSetter();
    }
    ...
    function valuesSetter() {
        ...
        document.querySelector(".calc_section .box.first .bottom").style.background = `linear-gradient(to top, #5ab9ea 0%, #5ab9ea ${per}%, transparent ${per}%, transparent 100%)`;
    }
    valuesSetter();
}
...
const map = () => {
    ...
    mapInfo.forEach((info, idx) => {
        ...
        button.addEventListener("click", (evt) => {
            ...
            targetPath.setAttribute('fill', '#c1c8e4');
            gsap.to(svg, {
                duration: 0.5,
                attr: {
    ...
    function setAllColors() {
        mapInfo.forEach(info => {
          ...
          if (pathElement) {
            pathElement.setAttribute('fill', '#c1c8e4');
          }
        });
    }
